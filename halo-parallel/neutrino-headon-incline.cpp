...
int main(int argc, char *argv[]) { // seven arguments: { range, reflection coefficient, neutrino potential mu, damping coefficient, # of total iteractions, # of steps within one calculations, # of threads }
     ...
     */
     if (argc < 8) {
        ...
        cerr << "Usage: " << argv[0] << " takes 7 parameters:  { range, reflection coefficient, neutrino potential mu, damping coefficient, # of total iteractions, # of steps within one calculations, # of threads }" << endl;
    ...
    */
    ...
    const double DALPHA = stod(argv[4]); // Set reflection coefficient
    const int Ntop = stoi(argv[5]); // Set how many times the overall iteraction is
    const int STEPS = stoi(argv[6]); // Set size of the array of z in z direction
    const int TH = stoi(argv[7]); // Set number of threads
    ...
    const int STEPSAVE = Ntop/3000; // How many overall iteractions before saving data
    ...
    const double incline_alpha = DALPHA; // within [0,1]
    ...
    */
    ...
    /* 
        data for saving
    */
    ofstream data("neutrino-headon-incline-" + to_string(incline_alpha) + "_MU_" + to_string(MIU) +"_REFL" + to_string(RCOEFF) + "_ITER" + to_string(Ntop)+"_STEPS"+ to_string(STEPS) + "_RANGE" + to_string(range1) + "_TH_" + to_string(TH) +"_t" + tmg.timestamp() + ".csv" ); 
    ...
    */
    ...
    cout << "Incline coefficient alpha: " << to_string(incline_alpha) << endl;
    ...
    */
    for(int i=0; i<Ntop; i++ ){
        ...
        // Evolve
       //if( (Ntop/3 <= i) && (i < 2*Ntop/3)  ) {
                Looper::halo_euler_forward_one_incline(sa_ptr, sa_store_ptr, STEPSIZE, SIZE, incline_alpha, RCOEFF, MIU);
       //}
       //         else {
       //Looper::halo_euler_forward_one(sa_ptr, sa_store_ptr, STEPSIZE, SIZE, RCOEFF, MIU);
       //}
