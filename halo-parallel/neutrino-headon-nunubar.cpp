...
int main(int argc, char *argv[]) { // six arguments: { range, reflection coefficient, neutrino potential mu, # of total iteractions, # of steps within one calculations, # of threads }
    ...
    */
    ...
    const int STEPSAVE = Ntop/30; // How many overall iteractions before saving data
    ...
    const int STEPSAVEZ = SIZE/1000; // How many steps before save a data point within each iteraction
    ...
    */
    double init_value[3] = {0.001,0.0,1.0};
    ...
    Recorder::record_z(z_array, SIZE-1, data, STEPSAVEZ);
    ...
    */
    for(int i=0; i<Ntop; i++ ){
        ...
        Looper::halo_euler_forward_one_nunubar(sa_ptr, sa_store_ptr, ya_sa_ptr, ya_sa_store_ptr, STEPSIZE, SIZE, spect, RCOEFF, mu_arr, costheta_arr);
        // Looper::halo_euler_forward_one_bipolar(sa_ptr, sa_store_ptr, ya_sa_ptr, ya_sa_store_ptr, STEPSIZE, SIZE, spect, RCOEFF, mu_arr, costheta_arr);
