...
int main(int argc, char *argv[]) { // six arguments: { range, reflection coefficient, neutrino potential mu, # of total iteractions, # of steps within one calculations, # of threads }
    ...
    */
    ...
    const int STEPSAVE = Ntop/100; // How many overall iteractions before saving data
    ...
    */
    ...
    /* 
        data for saving
    */
    ofstream data("neutrino-headon_MU_" + to_string(MIU) +"_REFL" + to_string(RCOEFF) + "_s2t" + to_string(s2theta) + "omegav_" + to_string(omegav) + "_ITER" + to_string(Ntop)+"_STEPS"+ to_string(STEPS) + "_RANGE" + to_string(range1) + "_TH_" + to_string(TH) +"_t" + tmg.timestamp() + ".csv" ); 
    ...
    */
    ...
    state_type rho_init = { 1e-3, 0.0, 0.5 };
    ...
    */
    double init_value[3] = {0.0,0.0,0.5};
